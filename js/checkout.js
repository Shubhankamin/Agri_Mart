document.addEventListener("DOMContentLoaded", function () {
  ...
  function setupMobileMenu() {
    ...
    if (currentUser) {
      ...
      if (mobileUserName)
        mobileUserName.textContent = currentUser.name || "User";
      if (mobileUserEmail)
  ...
  if (!currentUserStr) {
    ...
    const savedAddresses = addresses.filter((addr) => {
      console.log("Address email:", addr.email);
      console.log("Current user email:", currentUser.email);
      return addr.email === currentUser.email;
    });
  ...
  function renderSavedAddresses() {
    ...
    if (savedAddresses.length > 0 && !selectedShippingAddress) {
      ...
      const addressExists = savedAddresses.find(
  ...
  function updateNavbarCartCount() {
    ...
    // ✅ Also persist latest count (optional but good for quick access)
  ...
  function setupEventListeners() {
    ...
    differentBillingRadio.addEventListener("change", function () {
      if (this.checked) {
        ...
        selectedDeliveryPreview.innerHTML =
          '<p class="no-addresses">Please select or add a billing address below.</p>';
        ...
        announceToScreenReader(
          "Use different billing address selected. Please add a billing address if needed."
        );
      }
    });
  ...
  function handleAddressSubmit(e) {
    ...
    const addressData = {
      ...
      type: document.querySelector('input[name="edit-address-type"]:checked')
        .value,
    ...
    if (isAddingNewAddress) {
      ...
      if (index !== -1) {
        ...
        const profileIndex = allAddresses.findIndex(
          (a) => a.id == editingAddressId
        );
        if (profileIndex !== -1) {
          allAddresses[profileIndex] = {
            ...allAddresses[profileIndex],
            ...addressData,
          };
        }
    ...
    if (isAddingNewAddress) {
      ...
      const updatedAddress = savedAddresses.find(
        (a) => a.id == editingAddressId
      );
  ...
  function createOrder() {
    ...
    const shippingAddr =
      selectedShippingAddress ||
      savedAddresses.find((a) => a.email === currentUser.email) ||
      null;
    const billingAddr =
      selectedBillingAddress ||
      billingAddresses.find((a) => a.email === currentUser.email) ||
      shippingAddr;
